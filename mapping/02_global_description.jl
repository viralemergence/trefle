...
pl1 = GeoMakie.surface!(ga1, longitudes(richness), latitudes(richness), transpose(replace(zhc.grid, nothing => NaN)); shading=false, interpolate=false, colormap=:lapaz)
ga2 = GeoAxis(fig[1, 3]; dest="+proj=$(_proj)", coastlines=true, title="Hosts gained post-imputation\n\n")
pl2 = GeoMakie.surface!(ga2, longitudes(richness), latitudes(richness), transpose(replace((zht - zhc).grid, nothing => NaN)); shading=false, interpolate=false, colormap=:bamako)
ga3 = GeoAxis(fig[2, 2]; dest="+proj=$(_proj)", coastlines=true, title="Zoonotic viruses pre-imputation\n\n")
pl3 = GeoMakie.surface!(ga3, longitudes(richness), latitudes(richness), transpose(replace(zvc.grid, nothing => NaN)); shading=false, interpolate=false, colormap=:lapaz)
ga4 = GeoAxis(fig[2, 3]; dest="+proj=$(_proj)", coastlines=true, title="Viruses gained post-imputation\n\n")
pl4 = GeoMakie.surface!(ga4, longitudes(richness), latitudes(richness), transpose(replace((zvt - zvc).grid, nothing => NaN)); shading=false, interpolate=false, colormap=:bamako)
cb1 = Colorbar(fig[1, 1], pl1; height=Relative(0.55))
cb2 = Colorbar(fig[1, 4], pl2; height=Relative(0.55))
cb3 = Colorbar(fig[2, 1], pl3; height=Relative(0.55))
cb4 = Colorbar(fig[2, 4], pl4; height=Relative(0.55))
datalims!(ga1)
datalims!(ga2)
datalims!(ga3)
datalims!(ga4)
